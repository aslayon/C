...
char Label[10][100];
int Label_loc[10] = { -1, };
int Label_loc_2[10] = { -1, };
int Label_n = 0;
char symbol[10][100];
int symbol_loc[10] = { -1, };
int symbol_loc_2[10] = { -1, };
int symbol_n = 0;
...
int is_start_with_label(char* string) {
    for (int i = 0; i < Label_n; i++) {
        if (strcmp(string, "MAIN") == 0) return -1;
        if (strcmp(string, "ASSUME") == 0) return -1;
...
int rtn_sl_loc_2(char* string) {
    for (int i = 0; i < symbol_n; i++) {
        if (strcmp(string, symbol[i]) == 0) {
            return symbol_loc_2[i];
        }
    }
    for (int i = 0; i < Label_n; i++) {
        if (strcmp(string, Label[i]) == 0) {
            return Label_loc_2[i];
        }
    }
    return -1;
}
            ...
            if (str[i] != ' ' && str[i] != '\n' && str[i] != '\t' && str[i] != '\r' && str[i] != '.' && str[i] != ',' && str[i] != ':' && flag == 0) {
                ...
                if (s_flag == 1 && count == 2) {
                    ...
                    if (strcmp(tmp, "DB") == 0 || strcmp(tmp, "DW") == 0) {
                        ...
                        symbol_loc_2[symbol_n] = count_stack;
                        ...
                        if (dupl_flag == 1) {}
                            ...
                            Label_loc_2[Label_n] = count_stack;
...
void read() {
    ...
    while (!feof(fp)) {
        ...
        printf("\t");
        for (int i = 0; i < a; i++) {
            if (i == 0 && is_start_with_label(words[i])!=-1) {
                continue;
            }
            else {
                printf("%s\t", words[i]);
        ...
        printf("%04x\t", count_stack);
        for (int i = 0; i < a; i++) {
           
            if (is_cmd(words[i]) == 1) {
                if (strcmp(words[i], "MOV") == 0) {
                    printf("%02x\t",139);
                    continue;
                }
                ...
                if (strcmp(words[i], "CMP") == 0) {
                    printf("%02x\t", 59);
                    continue;
                }
                if (strcmp(words[i], "JMP") == 0) {
                    printf("%02x\t", 235);
                    continue;
                }
                if (strcmp(words[i], "JA") == 0) {
                    printf("%02x\t", 119);
                    continue;
                }
                if (strcmp(words[i], "JE") == 0) {
                    printf("%02x\t", 116);
                    continue;
                }
                if (strcmp(words[i], "JB") == 0) {
                    printf("%02x\t", 114);
                    continue;
                }
            }
            if (strcmp(words[i], "SEGMENT") == 0) {
                printf("[Direc]\t");
                continue;
            }
            if (strcmp(words[i], "DB") == 0) {
                printf("[의사]\t");
                continue;
            }
            if (strcmp(words[i], "DW") == 0) {
                printf("[의사]\t");
                continue;
            }
            if (is_r_OR_num(words[i]) == 1) {
                printf("%02x\t", rtn_r_wd(words[i]));
                continue;
            }
            if (is_r_OR_num(words[i]) == 2) {
                printf("%02x\t", rtn_r_bt(words[i]));
                continue;
            }
            if (is_r_OR_num(words[i]) == 3) {
                printf("[Numb]\t");
                continue;
            }
            if (is_r_OR_num(words[i]) == 0) {
                printf("%02x\t",rtn_sl_loc_2(words[i]));
                continue;
            }
        }
        count_stack += count;
        ...
        if (main_start_flag == 1) {
            ...
            if (strcmp(words[k + 0], "MOV") == 0) {
                ...
                if (head == 1 && tail == 4) {
                    ...
                    for (int j = 0; j < strlen(words[k + 2]); j++) {
                        words[k + 2][j] = words[k + 2][j + 1];
                    }
                    words[k + 2][strlen(words[k + 2])-1] = '\0';
                ...
                if (head == 4 && tail == 1) {
                    ...
                    for (int j = 0; j < strlen(words[k + 2]); j++) {
                        words[k + 1][j] = words[k + 1][j + 1];
                    }
                    words[k + 1][strlen(words[k + 1]) - 1] = '\0';
